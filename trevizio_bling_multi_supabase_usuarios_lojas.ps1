...
function Find-User($usuario) {
    $rows = @(Invoke-SupabaseGet ("usuarios?select=*&usuario=eq.{0}" -f [uri]::EscapeDataString([string]$usuario)))
    if($rows.Count -gt 0){
        $u = $rows[0]
        return [pscustomobject]@{
            usuario = $u.usuario
...
function Upsert-Account($alias, $clientId, $clientSecret) {
    ...
    $payload = [pscustomobject]@{
        ...
        expires_at = $(if($existing){$existing.expires_at}else{""})
...
if($path -match "^/api/order/(.+?)/(.+)$"){
            ...
            if($path -eq "/api/clear" -and $req.HttpMethod -eq "POST"){
                $null = Require-Admin $req
                foreach($p in @($ordersPath,$agesPath,$pendingPath)){ if(Test-Path $p){ Remove-Item $p -Force } }
                Send-Json $ctx @{ok=$true}
                continue
            }
