...
function Ensure-UsersFile {
    $users = Read-JsonFile $usersPath @()
    if($null -eq $users -or ($users | Measure-Object).Count -eq 0){
        $users = Get-DefaultUsers
        Write-JsonFile $usersPath $users
    }
    return @($users)
}
function Get-Users {
    return @(Ensure-UsersFile)
}
...
function Find-User($usuario) {
    foreach($u in (Get-Users)) { if([string]$u.usuario -eq [string]$usuario){ return $u } }
    return $null
}
...
function Add-AppUser($usuario, $senha, $tipo, $nome) {
    ...
    $users = Get-Users
    $users += [pscustomobject]@{ usuario=$usuario; senha=$senha; tipo=$tipo; nome=($(if($nome){$nome}else{$usuario})); bloqueado=$false }
    Save-Users $users
}
function Update-AppUser($usuario, $senha, $tipo, $nome, $bloqueado) {
    $users = Get-Users
    $found = $false
    $adminsAtivos = @($users | Where-Object { $_.tipo -eq 'admin' -and -not $_.bloqueado })
    $new = @()
    foreach($u in $users){
        if([string]$u.usuario -eq [string]$usuario){
            $found = $true
            if($senha){ $u.senha = $senha }
            if($tipo){ $u.tipo = $tipo }
            if($nome){ $u.nome = $nome }
            if($null -ne $bloqueado){ $u.bloqueado = [bool]$bloqueado }
            $new += $u
        } else {
            $new += $u
        }
    }
    if(-not $found){ throw 'Usuário não encontrado.' }
    $adminsAtivosDepois = @($new | Where-Object { $_.tipo -eq 'admin' -and -not $_.bloqueado })
    if($adminsAtivosDepois.Count -lt 1){ throw 'Deve existir pelo menos um admin ativo.' }
    Save-Users $new
}
function Delete-AppUser($usuario) {
    if([string]$usuario -eq 'admin'){ throw 'O admin principal não pode ser excluído.' }
    $users = Get-Users
    $new = @($users | Where-Object { [string]$_.usuario -ne [string]$usuario })
    if($new.Count -eq $users.Count){ throw 'Usuário não encontrado.' }
    $adminsAtivos = @($new | Where-Object { $_.tipo -eq 'admin' -and -not $_.bloqueado })
    if($adminsAtivos.Count -lt 1){ throw 'Deve existir pelo menos um admin ativo.' }
    Save-Users $new
