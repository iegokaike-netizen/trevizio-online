...
$script:PaymentMethodsCache = @{}

function Normalize-PaymentText([object]$value) {
    ...
    if($txt -match '\|'){
        $normParts = New-Object System.Collections.ArrayList
        foreach($part in ($txt -split '\|')){
            $p = Normalize-PaymentText $part
            if(-not [string]::IsNullOrWhiteSpace($p) -and -not $normParts.Contains($p)){ [void]$normParts.Add($p) }
        }
        if($normParts.Count -gt 0){ return (@($normParts) -join ' | ') }
    }

    $low = $txt.ToLowerInvariant()
    $isMercadoPago = ($low -match 'mercado\s*pago|mercadopago')
    if($low -match 'pix|chave pix|qr ?code'){ return 'PIX' }
    if($low -match 'boleto|ficha de compensa') { return 'Boleto' }
    if($low -match 'cr[eé]dito|credito') { if($isMercadoPago){ return 'Cartão de crédito Mercado Pago' } return 'Cartão de crédito' }
    if($low -match 'd[eé]bito|debito') { if($isMercadoPago){ return 'Cartão de débito Mercado Pago' } return 'Cartão de débito' }
    if($isMercadoPago -and $low -match 'cart[aã]o|visa|master|elo|amex|maquininha'){ return 'Cartão Mercado Pago' }
    if($low -match 'cart[aã]o|visa|master|elo|amex|maquininha') { return 'Cartão' }
    if($isMercadoPago){ return 'Mercado Pago' }
    if($low -match 'transfer|ted|doc') { return 'Transferência' }
    if($low -match 'dinheiro') { return 'Dinheiro' }
    return $txt
}

function Get-BlingPaymentMethods($alias) {
    if([string]::IsNullOrWhiteSpace([string]$alias)){ return @{} }
    if($script:PaymentMethodsCache.ContainsKey($alias)){ return $script:PaymentMethodsCache[$alias] }

    $map = @{}
    try {
        $headers = Get-AuthHeader $alias
        foreach($url in @(
            'https://api.bling.com.br/Api/v3/formas-pagamentos?pagina=1&limite=100',
            'https://api.bling.com.br/Api/v3/formas-pagamento?pagina=1&limite=100'
        )){
            try {
                $resp = Invoke-RestMethod -Method Get -Uri $url -Headers $headers
                $items = @()
                if($resp.data){ $items = @($resp.data) }
                elseif($resp.itens){ $items = @($resp.itens) }
                elseif($resp){ $items = @($resp) }
                foreach($it in $items){
                    if($it -and $it.id){
                        $name = ''
                        foreach($field in @('descricao','nome','forma','tipo')){
                            if($it.PSObject.Properties.Name -contains $field -and $it.$field){ $name = [string]$it.$field; break }
                        }
                        if($name){ $map[[string]$it.id] = $name }
                    }
                }
    ...
    $script:PaymentMethodsCache[$alias] = $map
    return $map
}

function Get-PaymentTextFromOrder($alias, $raw) {
    $parts = New-Object System.Collections.ArrayList
    $ids = New-Object System.Collections.ArrayList

    function Add-PaymentPart([object]$v) {
        if($null -eq $v){ return }
        $txt = [string]$v
        if([string]::IsNullOrWhiteSpace($txt)){ return }
        $txt = $txt.Trim()
        if($txt -match '^[\{\[]'){ return }
        if($txt -match '^\d+$'){
            if(-not $ids.Contains($txt)){ [void]$ids.Add($txt) }
            return
        }
        if(-not $parts.Contains($txt)){ [void]$parts.Add($txt) }
    }

    function Read-PaymentObject([object]$obj) {
        if($null -eq $obj){ return }
        if($obj -is [string] -or $obj -is [int] -or $obj -is [long]){ Add-PaymentPart $obj; return }
        if($obj -is [pscustomobject]){
            foreach($name in @('descricao','nome','forma','tipo','codigo','codigoFiscal','bandeira','observacao','observacoes','obs')){
                if($obj.PSObject.Properties.Name -contains $name){ Add-PaymentPart $obj.$name }
            }
            if($obj.PSObject.Properties.Name -contains 'id'){
                $idTxt = [string]$obj.id
                if(-not [string]::IsNullOrWhiteSpace($idTxt) -and -not $ids.Contains($idTxt)){ [void]$ids.Add($idTxt) }
            }
        }
    }

    try {
        if($null -eq $raw){ return 'Não informado' }

        foreach($field in @('pagamento','formaPagamento','forma_pagamento','meioPagamento','meio_pagamento','paymentMethod','payment_method','condicaoPagamento','condicao_pagamento')){
            foreach($v in (Find-DeepValuesByFieldName $raw $field)){ Read-PaymentObject $v }
        }

        foreach($parcelasField in @('parcelas','pagamentos')){
            foreach($arr in (Find-DeepValuesByFieldName $raw $parcelasField)){
                foreach($parcela0 in @($arr)){
                    $p = $parcela0
                    if($p -and ($p -is [pscustomobject]) -and $p.PSObject.Properties.Name -contains 'parcela' -and $p.parcela){ $p = $p.parcela }
                    Read-PaymentObject $p
                    if($p -and ($p -is [pscustomobject])){
                        foreach($f in @('formaPagamento','forma_pagamento','meioPagamento','meio_pagamento','pagamento')){
                            if($p.PSObject.Properties.Name -contains $f){ Read-PaymentObject $p.$f }
                        }
                    }
                }
            }
        ...
        if($ids.Count -gt 0){
            $map = Get-BlingPaymentMethods $alias
            foreach($id in @($ids)){
                if($map.ContainsKey([string]$id)){ Add-PaymentPart $map[[string]$id] }
            }
        }

        if($parts.Count -eq 0){ return 'Não informado' }
        $joined = (@($parts) -join ' | ')
        $normalized = Normalize-PaymentText $joined
...
function Find-PropValueDeep($obj, [string[]]$names, [int]$depth=0) {
    if($null -eq $obj -or $depth -gt 8){ return $null }

    if($obj -is [string] -or $obj.GetType().IsPrimitive){ return $null }

    if($obj -is [System.Collections.IEnumerable] -and -not ($obj -is [string])){
        foreach($x in @($obj)){
            $found = Find-PropValueDeep $x $names ($depth + 1)
            if($null -ne $found -and -not [string]::IsNullOrWhiteSpace([string]$found)){ return $found }
        }
        return $null
    }
    ...
    foreach($n in $names){
        foreach($p in $obj.PSObject.Properties){
            if((Normalize-CanalText $p.Name) -eq (Normalize-CanalText $n)){
                if($null -ne $p.Value -and -not [string]::IsNullOrWhiteSpace([string]$p.Value)){ return $p.Value }
            }
        }
    }

    foreach($p in $obj.PSObject.Properties){
        $found = Find-PropValueDeep $p.Value $names ($depth + 1)
        if($null -ne $found -and -not [string]::IsNullOrWhiteSpace([string]$found)){ return $found }
    }
    return $null
}
...
function Add-AllTextParts($obj, [System.Collections.ArrayList]$parts, [int]$depth=0) {
    if($null -eq $obj -or $depth -gt 10){ return }
    ...
    if($obj -is [System.Collections.IEnumerable] -and -not ($obj -is [string])){
        foreach($x in @($obj)){ Add-AllTextParts $x $parts ($depth + 1) }
        return
    }

    foreach($prop in $obj.PSObject.Properties){
        if($prop.Name){ [void]$parts.Add([string]$prop.Name) }
        Add-AllTextParts $prop.Value $parts ($depth + 1)
    }
}

function Get-CanalCodigo($alias, $obj) {
    if($null -eq $obj){ return 'OUTROS' }

    $parts = New-Object System.Collections.ArrayList
    # Importante: alias da conta não é usado para decidir canal.
    # Uma mesma loja Bling pode vender em Mercado Livre e Shopee ao mesmo tempo.
    Add-CanalTextParts $obj $parts 0
    $canal = Get-CanalCodigoFromText (($parts | Where-Object { $_ -and -not [string]::IsNullOrWhiteSpace([string]$_) }) -join ' ')
    if($canal -ne 'OUTROS'){ return $canal }

    # Fallback forte: em algumas respostas do Bling o texto vem em campos fora do padrão
    # (ex.: observação interna, texto de DANFE, metadados ou objeto aninhado). Quando o
    # campo certo muda, a versão online caía em OUTROS. Aqui varremos o JSON inteiro.
    $all = New-Object System.Collections.ArrayList
    Add-AllTextParts $obj $all 0
    return Get-CanalCodigoFromText (($all | Where-Object { $_ -and -not [string]::IsNullOrWhiteSpace([string]$_) }) -join ' ')
}
...
function Get-LojaRawForChannel($alias, $lojaId) {
    ...
    $tentativas = @(
        ...
        "https://api.bling.com.br/Api/v3/lojas-virtuais/$encodedId",
        "https://api.bling.com.br/Api/v3/canais-venda/$encodedId",
        "https://api.bling.com.br/Api/v3/canais-vendas/$encodedId",
        "https://api.bling.com.br/Api/v3/integracoes/$encodedId",
        "https://api.bling.com.br/Api/v3/lojas?ids%5B%5D=$encodedId",
        "https://api.bling.com.br/Api/v3/lojas/virtuais?ids%5B%5D=$encodedId",
        "https://api.bling.com.br/Api/v3/lojas-virtuais?ids%5B%5D=$encodedId",
        "https://api.bling.com.br/Api/v3/canais-venda?ids%5B%5D=$encodedId",
        "https://api.bling.com.br/Api/v3/canais-vendas?ids%5B%5D=$encodedId",
        "https://api.bling.com.br/Api/v3/integracoes?ids%5B%5D=$encodedId",
        ...
        "https://api.bling.com.br/Api/v3/canais-vendas",
        "https://api.bling.com.br/Api/v3/integracoes"
    )
...
function Get-LojaIdFromOrder($obj) {
    ...
    if($loja){
        ...
        $id = Get-PropValue $loja @('id','codigo','código','numero','idLoja','lojaId')
        if($id){ return [string]$id }
    }
    $id2 = Get-PropValue $obj @('lojaId','loja_id','idLoja','id_loja','numeroLoja','numero_loja')
    if($id2){ return [string]$id2 }
    $id3 = Find-PropValueDeep $obj @('lojaId','loja_id','idLoja','id_loja','numeroLoja','numero_loja')
    if($id3){ return [string]$id3 }
    return ""
}

function Get-NumeroPedidoLojaFromOrder($obj) {
    if($null -eq $obj){ return "" }
    $v = Get-PropValue $obj @(
        'numeroPedidoLoja','numero_pedido_loja','pedidoLoja','pedido_loja',
        'numeroPedidoMarketplace','numero_pedido_marketplace','codigoPedidoLoja','codigo_pedido_loja',
        'pedidoMarketplace','pedido_marketplace','orderSn','ordersn','order_id','orderId','marketplaceOrderId'
    )
    if($v){ return [string]$v }

    $deep = Find-PropValueDeep $obj @(
        'numeroPedidoLoja','numero_pedido_loja','pedidoLoja','pedido_loja',
        'numeroPedidoMarketplace','numero_pedido_marketplace','codigoPedidoLoja','codigo_pedido_loja',
        'pedidoMarketplace','pedido_marketplace','orderSn','ordersn','order_id','orderId','marketplaceOrderId'
    )
...
function Get-CanalCodigoByPedidoLojaPattern($obj) {
    $num = (Get-NumeroPedidoLojaFromOrder $obj).Trim().ToUpperInvariant()
    if([string]::IsNullOrWhiteSpace($num)){ return 'OUTROS' }
    $numLimpo = ($num -replace '[^A-Z0-9]', '')

    # Mercado Livre costuma trazer MLB/MELI/ML ou pedido longo numérico iniciando em 20/20...
    if($numLimpo -match '^MLB[0-9]+' -or $numLimpo -match '^MELI' -or $num -match '^ML[-_ ]?[0-9]+'){ return 'MERCADO_LIVRE' }
    if($numLimpo -match '^20[0-9]{10,}$'){ return 'MERCADO_LIVRE' }

    # Shopee costuma trazer código alfanumérico sem hífen, com letras e números.
    if($numLimpo -match '^[0-9]{6}[A-Z0-9]{6,}$' -and $numLimpo -match '[A-Z]'){ return 'SHOPEE' }
    if($numLimpo -match '^[A-Z0-9]{12,18}$' -and $numLimpo -match '[A-Z]' -and $numLimpo -match '[0-9]'){
        if($numLimpo -notmatch '^ML' -and $numLimpo -notmatch '^MELI'){ return 'SHOPEE' }
    }

    return 'OUTROS'
}

function Get-CanalCodigoFromSavedMap($alias, $lojaId, $pedidoLoja) {
    # Não reutiliza canal somente pelo lojaId, porque a mesma loja/conta pode vender
    # em Shopee e Mercado Livre. Só reaproveita quando o número do pedido da loja é igual.
    if(-not $pedidoLoja){ return 'OUTROS' }
    $orders = @()
    try { $orders = @(Get-Orders) } catch { return 'OUTROS' }
    foreach($o in $orders){
        $oc = [string]$o.canal
        if($oc -and $oc -ne 'OUTROS'){
            if($o.numeroPedidoLoja -and [string]$o.numeroPedidoLoja -eq [string]$pedidoLoja){ return $oc }
        }
    }
    return 'OUTROS'
}
...
function Get-CanalCodigoPedido($alias, $obj) {
    ...
    # 2) Usa padrão do número do pedido da loja quando o Bling não manda o nome do canal.
    $canalPedidoLoja = Get-CanalCodigoByPedidoLojaPattern $obj
    if($canalPedidoLoja -ne 'OUTROS'){ return $canalPedidoLoja }

    # 3) Se o pedido só trouxe ID da loja, busca a loja/integração/canal de venda no Bling.
    ...
    # 4) Mantém consistência: se outra nota com o mesmo ID de loja já foi classificada,
    # usa a mesma classificação em vez de jogar novamente em OUTROS.
    $pedidoLoja = Get-NumeroPedidoLojaFromOrder $obj
    $canalSalvo = Get-CanalCodigoFromSavedMap $alias $lojaId $pedidoLoja
    if($canalSalvo -ne 'OUTROS'){ return $canalSalvo }
...
function Normalize-Order($alias, $item) {
    ...
    $cliente=""; $cpf=""; $data=""; $valor=""; $nota=""; $idade=""; $pagamento=""
    ...
    $numeroPedidoLoja = Get-NumeroPedidoLojaFromOrder $item
    $canal = Get-CanalCodigoPedido $alias $item
    $pagamento = Get-PaymentTextFromOrder $alias $item
...
function Normalize-SyncDate($syncDate) {
    if([string]::IsNullOrWhiteSpace([string]$syncDate)){ return "" }
    $txt = ([string]$syncDate).Trim()
    try {
        $dt = [datetime]::ParseExact($txt, "yyyy-MM-dd", [System.Globalization.CultureInfo]::InvariantCulture)
...
function Get-BlingPedidosVendasPath($pagina, $syncDate) {
    $path = "/pedidos/vendas?pagina=$pagina&limite=100"
    $d = Normalize-SyncDate $syncDate
    if(-not [string]::IsNullOrWhiteSpace($d)){
        $path += "&dataInicial=$d&dataFinal=$d"
    }
    return $path
}

function Invoke-BlingGetPath($pathAndQuery, $headers) {
    if(-not ([string]$pathAndQuery).StartsWith('/')){ $pathAndQuery = '/' + [string]$pathAndQuery }
    $url = "https://api.bling.com.br/Api/v3$pathAndQuery"
    return Invoke-RestMethod -Method Get -Uri $url -Headers $headers
}

function Get-BlingItemsFromResponse($resp) {
    if($null -eq $resp){ return @() }
    if($resp.data){ return @($resp.data) }
    if($resp.itens){ return @($resp.itens) }
    if($resp.pedidos){ return @($resp.pedidos) }
    return @()
}

function Compare-OrderDateToSyncDate($row, $syncDate) {
    $d = Normalize-SyncDate $syncDate
    if([string]::IsNullOrWhiteSpace($d)){ return $null }
    if($null -eq $row){ return $null }
    $camposData = @('data','dataSaida','dataEmissao','dataCriacao','createdAt')
    foreach($campo in $camposData){
        try {
            $prop = $row.PSObject.Properties[$campo]
            if($prop -and -not [string]::IsNullOrWhiteSpace([string]$prop.Value)){
                $rowDate = Parse-OrderDate $prop.Value
                if($null -ne $rowDate){
                    $rowDay = $rowDate.ToString('yyyy-MM-dd')
                    if($rowDay -eq $d){ return 0 }
                    if($rowDay -lt $d){ return -1 }
                    if($rowDay -gt $d){ return 1 }
                }
            }
        } catch {}
    }
...
function Test-OrderMatchesSyncDate($row, $syncDate) {
    $d = Normalize-SyncDate $syncDate
    ...
    foreach($campo in $camposData){
        try {
            ...
            if($prop -and -not [string]::IsNullOrWhiteSpace([string]$prop.Value)){
                $rowDate = Parse-OrderDate $prop.Value
                if($null -ne $rowDate -and $rowDate.ToString('yyyy-MM-dd') -eq $d){ return $true }
            }
        } catch {}
    }
    return $false
...
function Should-StopPagingForSyncDate($items, $syncDate) {
    $d = Normalize-SyncDate $syncDate
    if([string]::IsNullOrWhiteSpace($d)){ return $false }
    if(-not $items -or @($items).Count -eq 0){ return $true }
    $temDiaEscolhido = $false
    $temMaisAntigo = $false
    $temMaisNovo = $false
    foreach($it in @($items)){
        $cmp = Compare-OrderDateToSyncDate $it $d
        if($null -eq $cmp){ continue }
        if($cmp -eq 0){ $temDiaEscolhido = $true }
        elseif($cmp -lt 0){ $temMaisAntigo = $true }
        elseif($cmp -gt 0){ $temMaisNovo = $true }
    }
    if($temDiaEscolhido){ return $true }
    if($temMaisAntigo -and -not $temMaisNovo){ return $true }
    return $false
}

function Sync-Accounts($aliases, $syncDate = "") {
    $all = @()
    $selected = @()
    if($aliases){
        $selected = @($aliases | ForEach-Object { ([string]$_).Trim() } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    }
    $selectedLower = @($selected | ForEach-Object { ([string]$_).Trim().ToLowerInvariant() })
    $syncDateNorm = Normalize-SyncDate $syncDate

    foreach($a in (Get-Accounts)){
        if(-not $a.access_token){ continue }
        $aliasAtual = ([string]$a.alias).Trim()
        if($selectedLower.Count -gt 0 -and -not ($selectedLower -contains $aliasAtual.ToLowerInvariant())){ continue }
        $headers = Get-AuthHeader $a.alias

        for($pagina=1; $pagina -le 50; $pagina++){
            try {
                $resp = Invoke-BlingGetPath (Get-BlingPedidosVendasPath $pagina $syncDateNorm) $headers
                $items = @(Get-BlingItemsFromResponse $resp)

                foreach($it in $items){
                    $ord = Normalize-Order $a.alias $it
                    if($ord -and $ord.pedidoId -and (Test-OrderMatchesSyncDate $ord $syncDateNorm)){
                        $precisaDetalhe = ($ord.canal -eq 'OUTROS' -or [string]::IsNullOrWhiteSpace([string]$ord.pagamento) -or [string]$ord.pagamento -eq 'Não informado')
                        if($precisaDetalhe -and $ord.pedidoId){
                            try{
                                $detailRaw = Get-OrderRawForChannel $a.alias $ord.pedidoId
                                if($ord.canal -eq 'OUTROS'){
                                ...
                                $pagamentoDetail = Get-PaymentTextFromOrder $a.alias $detailRaw
                                if(-not [string]::IsNullOrWhiteSpace([string]$pagamentoDetail) -and [string]$pagamentoDetail -ne 'Não informado'){
                                    $ord.pagamento = $pagamentoDetail
                                    $ord.formaPagamento = $pagamentoDetail
                                }
                            } catch {}
                        }
                        $all += $ord
                ...
                if((Should-StopPagingForSyncDate $items $syncDateNorm)){ break }
                if(-not $items -or @($items).Count -lt 100){ break }
                Start-Sleep -Milliseconds 400
            } catch {
                throw "Erro ao buscar loja $($a.alias), página ${pagina}: $($_.Exception.Message)"
            }
        }
    }

    $oldOrders = @(Get-Orders)
    if($syncDateNorm){
        $keep = @()
        foreach($o in $oldOrders){
            $aliasO = ([string]$o.alias).Trim().ToLowerInvariant()
            $lojaFoiSelecionada = ($selectedLower.Count -eq 0 -or ($selectedLower -contains $aliasO))
            if((-not $lojaFoiSelecionada) -or (-not (Test-OrderMatchesSyncDate $o $syncDateNorm))){
                $keep += $o
            }
        }
        Save-Orders (@($keep) + @($all))
    } elseif($selectedLower.Count -gt 0){
        $keep = @()
        foreach($o in $oldOrders){
            if(-not ($selectedLower -contains ([string]$o.alias).Trim().ToLowerInvariant())){ $keep += $o }
        }
        Save-Orders (@($keep) + @($all))
    } else {
        Save-Orders $all
...
function Sync-All($syncDate = "") {
    return Sync-Accounts -aliases @() -syncDate $syncDate
}
...
function Get-OrderDetail($alias, $pedidoId) {
    ...
    $cliente=""; $cpf=""; $data=""; $valor=""; $nota=""; $endereco=""; $itemDescricao=""; $rawHint=""; $idade=""; $canal=""; $pagamento=""
    $contactId = ""
    ...
    $numeroPedidoLoja = Get-NumeroPedidoLojaFromOrder $raw
    $canal = Get-CanalCodigoPedido $alias $raw
    $pagamento = Get-PaymentTextFromOrder $alias $raw
    ...
    while($listener.IsListening){
        ...
        try {
            ...
            if($path -eq "/api/sync-all"){
                $null = Require-Admin $req
                $syncDate = Normalize-SyncDate $req.QueryString["date"]
                $orders = Sync-All $syncDate
                Send-Json $ctx @{ok=$true; count=$orders.Count; syncDate=$syncDate}
                continue
            }
            if($path -eq "/api/sync-selected" -and $req.HttpMethod -eq "POST"){
                ...
                $syncDate = ""
                if($body -and $body.syncDate){ $syncDate = Normalize-SyncDate $body.syncDate }
                elseif($body -and $body.date){ $syncDate = Normalize-SyncDate $body.date }
                $orders = Sync-Accounts $aliases $syncDate
                Send-Json $ctx @{ok=$true; count=$orders.Count; aliases=$aliases; syncDate=$syncDate}
                continue
            }
...
if($path -eq "/api/ages-batch" -and $req.HttpMethod -eq "POST"){
    # Busca idades em LOTES pequenos (5 por vez) para não estourar limite da FDX
    # nem sofrer corte de requisição longa do Render. Cada lote é salvo antes de responder.
    try {
        $body = Read-BodyJson $req
        $startDate = Parse-OrderDate $body.startDate
        $endDate = Parse-OrderDate $body.endDate
        ...
        # CPFs que já falharam nesta rodada (mandados pelo frontend) não são re-tentados
        $ignorar = @()
        if($body -and $body.ignorar){ $ignorar = @($body.ignorar | ForEach-Object { [string]$_ }) }

        $alvo = $allOrders
        $aliasesFiltro = Get-AliasesFromBody $body
        if($aliasesFiltro.Count -gt 0){
            $alvo = @($allOrders | Where-Object { $aliasesFiltro -contains [string]$_.alias })
        }
        $alvo = @($alvo | Where-Object {
            $od = Parse-OrderDate $_.data
            $od -and $od -ge $startDate.Date -and $od -le $endDate
        })

        # Processa direto: máximo 5 pedidos ou 45 segundos por rodada.
        # Usa só arrays comuns do PowerShell (evita 'Argument types do not match' com List do .NET)
        $limiteLote = 5
        $inicio = Get-Date
        $ages = Get-Ages
        $encontrados = 0
        ...
        foreach($o in $alvo){
            if($mapa.Count -ge $limiteLote){ break }
            if(((Get-Date) - $inicio).TotalMilliseconds -gt 45000){ break }
            $chave = ([string]$o.alias).Trim() + '|' + ([string]$o.pedidoId).Trim()
            if($ignorar -contains $chave){ continue }
            if(-not ((Is-OrderAgeMissing $o) -and $o.cpf)){ continue }

            $cpfLimpo = ([string]$o.cpf) -replace '[^\d]', ''
            $idadeResp = Invoke-BuscarIdadeComRetry $cpfLimpo 3
        ...
        if($mapa.Count -eq 0){
            Send-Json $ctx @{ ok=$true; processados=0; encontrados=0; naoEncontrados=0; falhas=@(); restantes=0; concluido=$true }
            continue
        }

        # Aplica as mudanças no conjunto COMPLETO (não truncar outras lojas!)
        $novaLista = @()
        foreach($o in $allOrders){
            $chave = ([string]$o.alias).Trim() + '|' + ([string]$o.pedidoId).Trim()
            if($mapa.ContainsKey($chave)){ $novaLista += $mapa[$chave] } else { $novaLista += $o }
        }

        Write-JsonFile $agesPath $ages
        Save-Orders $novaLista
        ...
        foreach($o in $alvo){
            ...
            if($ignorar -contains $chave){ continue }
            if((Is-OrderAgeMissing $o) -and $o.cpf){ $restantes++ }
        }

        Send-Json $ctx @{
            ok = $true
            processados = $mapa.Count
            encontrados = $encontrados
            naoEncontrados = $naoEncontrados
            falhas = $falhas
            restantes = $restantes
            concluido = ($restantes -eq 0)
        }
    } catch {
