...
function Get-ContactAddress($alias, $contactId) {
    ...
    try {
        ...
        if($raw.endereco -and $raw.endereco.geral){
            return (Format-AddressFromGeneral $raw.endereco.geral)
        }
...
function Get-OrderDetail($alias, $pedidoId) {
    ...
    if($raw.transporte -and $raw.transporte.etiqueta){
        $endereco = Format-AddressFromEtiqueta $raw.transporte.etiqueta
    }
    ...
    if($endereco -eq "-"){
        if($rawHint){ $rawHint += " " }
        $rawHint += "Endereço não veio em transporte.etiqueta e também não foi localizado em contato.endereco.geral."
    }
