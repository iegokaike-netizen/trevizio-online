...
function Get-CanalCodigo($alias, $obj) {
    ...
    # Uma mesma conta/loja pode vender em Mercado Livre e Shopee ao mesmo tempo.
...
function Add-LojaTextParts($obj, [System.Collections.ArrayList]$parts, [int]$depth=0) {
    if($null -eq $obj -or $depth -gt 8){ return }
    ...
    if($obj -is [System.Collections.IEnumerable] -and -not ($obj -is [string])){
        foreach($x in @($obj)){ Add-LojaTextParts $x $parts ($depth + 1) }
        return
    }

    # Aqui é propositalmente mais amplo que Add-CanalTextParts, porque este
    # objeto já veio do endpoint de loja/loja virtual. O nome/descrição direto
    # da loja é justamente o que identifica Shopee, Mercado Livre, Mercado Shops etc.
    $keys = @(
        'nome','descricao','descrição','tipo','canal','origem','marketplace','plataforma','integracao','integração',
        'tipoIntegracao','tipo_integracao','loja','lojaVirtual','loja_virtual','ecommerce','intermediador','store','seller',
        'codigo','código','numero','id'
    )
    ...
    foreach($prop in $obj.PSObject.Properties){
        ...
        $usar = $false
        foreach($k in $keys){
            $kn = Normalize-CanalText $k
            if($propName -eq $kn -or $propName -like "*$kn*"){ $usar = $true; break }
        }
        if($usar){ Add-LojaTextParts $prop.Value $parts ($depth + 1) }
    }
}

function Get-CanalCodigoLojaRaw($lojaRaw) {
    $parts = New-Object System.Collections.ArrayList
    Add-LojaTextParts $lojaRaw $parts 0
    return Get-CanalCodigoFromText (($parts | Where-Object { $_ -and -not [string]::IsNullOrWhiteSpace([string]$_) }) -join ' ')
}
...
function Get-CanalCodigoPedido($alias, $obj) {
    ...
    if($lojaId){
        try{
            ...
            $canalLoja = Get-CanalCodigoLojaRaw $lojaRaw
