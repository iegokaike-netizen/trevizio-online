...
if($path -eq "/api/ages-batch" -and $req.HttpMethod -eq "POST"){
    ...
    try {
        ...
        # Monta o lote: só quem ainda está sem idade, máximo 5 ou 40 segundos
        $limiteLote = 5
        $cronometro = [System.Diagnostics.Stopwatch]::StartNew()
        $lote = @()
        foreach($o in $alvo){
            $chaveO = ([string]$o.alias).Trim() + '|' + ([string]$o.pedidoId).Trim()
            if($ignorar -contains $chaveO){ continue }
            if((Is-OrderAgeMissing $o) -and $o.cpf){
                $lote += $o
                if($lote.Count -ge $limiteLote -or $cronometro.ElapsedMilliseconds -gt 40000){ break }
            }
        }

        if($lote.Count -eq 0){
            Send-Json $ctx @{ ok=$true; processados=0; encontrados=0; restantes=0; concluido=$true }
            continue
        }
        ...
        foreach($o in $lote){
            ...
            if($null -ne $idadeResp){
                ...
                $naoEncontrados++
                $falhas += (([string]$o.alias).Trim() + '|' + ([string]$o.pedidoId).Trim())
            }
        ...
        foreach($o in $allOrders){
            $chave = ([string]$o.alias).Trim() + '|' + ([string]$o.pedidoId).Trim()
            if($mapa.ContainsKey($chave)){ $novaLista.Add($mapa[$chave]) } else { $novaLista.Add($o) }
        }
