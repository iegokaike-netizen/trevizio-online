...
function Is-OrderAgeMissing($order) {
    ...
    if($order.PSObject.Properties.Name -contains "idade"){
        return [string]::IsNullOrWhiteSpace([string]$order.idade)
    }
    return $true
}
...
function BuscarIdadeCPF($cpf) {
    ...
    try {
        ...
        if([string]::IsNullOrWhiteSpace([string]$nasc)){
            $nasc = Find-FirstFieldValue $resp @("NASC", "NASCIMENTO", "dataDeNascimento", "data_de_nascimento")
        }
...
function Apply-AgesToOrders($orders) {
    ...
    foreach($o in @($orders)){
        if($o){
            $idadeAtual = $null
            if($o.PSObject.Properties.Name -contains 'idade'){ $idadeAtual = $o.idade }
            if([string]::IsNullOrWhiteSpace([string]$idadeAtual)){
                $idadeSalva = Get-AgeValue $o.alias $o.cpf $o.cliente
                if($null -ne $idadeSalva -and -not [string]::IsNullOrWhiteSpace([string]$idadeSalva)){
                    if($o.PSObject.Properties.Name -contains 'idade'){ $o.idade = [string]$idadeSalva }
                    else { $o | Add-Member -NotePropertyName idade -NotePropertyValue ([string]$idadeSalva) -Force }
                }
            }
        }
        $out += $o
...
function Get-CanalCodigo($alias, $obj) {
    $parts = @([string]$alias)
    if($obj){
        foreach($name in @('loja','marketplace','origem','canal','ecommerce','intermediador','numeroLoja')){
            if($obj.PSObject.Properties.Name -contains $name -and $obj.$name){
                $v = $obj.$name
                if($v -is [string]){ $parts += $v }
                else{
                    foreach($sub in @('nome','descricao','codigo','id','canal','origem')){
                        if($v.PSObject.Properties.Name -contains $sub -and $v.$sub){ $parts += [string]$v.$sub }
                    }
                }
            }
        }
    }
    $txt = Normalize-CanalText ($parts -join ' ')
    if($txt -match 'SHOPEE'){ return 'SHOPEE' }
    if($txt -match 'MERCADO\s*LIVRE' -or $txt -match 'MERCADOLIVRE' -or $txt -match '\bMELI\b' -or $txt -match '\bML\b'){ return 'MERCADO_LIVRE' }
    return 'OUTROS'
}
...
function Normalize-Order($alias, $item) {
    ...
    $canal = Get-CanalCodigo $alias $item
...
function Get-OrderDetail($alias, $pedidoId) {
    ...
    $idade = Get-AgeValue $alias $cpf $cliente
    if($null -eq $idade){ $idade = "" }
    $canal = Get-CanalCodigo $alias $raw
...
if($path -match "^/api/order/(.+?)/(.+)$"){
            ...
            if($path -eq "/api/age" -and $req.HttpMethod -eq "POST"){
                ...
                foreach($o in $orders){
                    ...
                    $cpfB = ([string]$body.cpf) -replace '[^\d]', ''
                    if($o.alias -eq $body.alias -and ((($cpfO) -and $cpfO -eq $cpfB) -or ((-not $cpfO) -and $o.cliente -eq $body.cliente))){
                        Set-OrderAge $o $body.idade
                    }
                    $updated += $o
                }
                Save-Orders $updated
