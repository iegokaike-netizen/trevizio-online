...
function Join-AddressParts($obj) {
    if(-not $obj){ return "" }
    $parts=@()
    foreach($f in @("endereco","logradouro","rua")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $parts += [string]$obj.$f; break } }
    foreach($f in @("numero")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $parts += [string]$obj.$f; break } }
    foreach($f in @("complemento")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $parts += [string]$obj.$f } }
    foreach($f in @("bairro")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $parts += [string]$obj.$f; break } }
    $city=@()
    foreach($f in @("municipio","cidade")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $city += [string]$obj.$f; break } }
    foreach($f in @("uf","estado")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $city += [string]$obj.$f; break } }
    if($city.Count -gt 0){ $parts += ($city -join "/") }
    foreach($f in @("cep")){ if($obj.PSObject.Properties.Name -contains $f -and $obj.$f){ $parts += "CEP " + [string]$obj.$f; break } }
    return ($parts -join ", ")
}
...
function Get-ContactAddress($alias, $contactId) {
    ...
    try {
        ...
        if($raw.endereco){ $candidates += $raw.endereco }
        if($raw.endereco -and $raw.endereco.geral){ $candidates += $raw.endereco.geral }
        if($raw.contato){ $candidates += $raw.contato }
        if($raw.contato -and $raw.contato.endereco){ $candidates += $raw.contato.endereco }
        if($raw.enderecoEntrega){ $candidates += $raw.enderecoEntrega }
        if($raw.dadosAdicionais){ $candidates += $raw.dadosAdicionais }

        foreach($field in @("geral","endereco","enderecoEntrega","destinatario")){
            foreach($item in (Find-DeepValuesByFieldName $raw $field)) {
                if($item){ $candidates += $item }
            }
        }

        foreach($cand in $candidates){
            $addr = Join-AddressParts $cand
            if($addr){ return $addr }
        }

        $parts = @()
        $rua = Find-FirstFieldValue $raw @("logradouro","rua")
        if($rua){ $parts += [string]$rua }

        $numero = Find-FirstFieldValue $raw @("numero")
        if($numero){ $parts += [string]$numero }

        $complemento = Find-FirstFieldValue $raw @("complemento")
        if($complemento){ $parts += [string]$complemento }

        $bairro = Find-FirstFieldValue $raw @("bairro")
        if($bairro){ $parts += [string]$bairro }

        $cidade = Find-FirstFieldValue $raw @("municipio","cidade")
        $uf = Find-FirstFieldValue $raw @("uf","estado")
        if($cidade -and $uf){ $parts += ("{0}/{1}" -f $cidade, $uf) }
        elseif($cidade){ $parts += [string]$cidade }
        elseif($uf){ $parts += [string]$uf }

        $cep = Find-FirstFieldValue $raw @("cep")
        if($cep){ $parts += ("CEP " + [string]$cep) }
...
function Get-OrderDetail($alias, $pedidoId) {
    ...
    # 2) tenta outros blocos do próprio pedido
    if(-not $endereco){
        foreach($cand in @($raw.enderecoEntrega, $raw.endereco, $raw.contato, $raw.destinatario, $raw.cliente)){
            if($cand){
                $addr = Join-AddressParts $cand
                if($addr){
                    $endereco = $addr
    ...
    # 3) fallback pelo contato (Mercado Livre)
    if(-not $endereco){
